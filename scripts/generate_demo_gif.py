...
ax.text(0.5, 0.6, 'nerf-gs-playground', fontsize=32, ha='center', color='white', fontweight='bold')
...
for i, (step, color) in enumerate(zip(steps, step_colors)):
    x = 0.1 + i * 0.2
    rect = plt.Rectangle((x-0.07, 0.35), 0.14, 0.3, facecolor=color, alpha=0.15, edgecolor=color, linewidth=2, transform=ax.transAxes)
    ax.add_patch(rect)
    ax.text(x, 0.5, step, ha='center', va='center', fontsize=12, fontweight='bold', color=color, transform=ax.transAxes)
    if i < len(steps) - 1:
        ax.annotate('', xy=(x+0.1, 0.5), xytext=(x+0.06, 0.5), xycoords='axes fraction', textcoords='axes fraction',
...
for scene, dir_path in scene_dirs.items():
    ...
    for img_path in imgs[:1]:
        if img_idx < 4:
            ...
            axes[img_idx].set_title(scene_labels.get(scene, scene), fontsize=10, color=scene_colors.get(scene, '#666'), fontweight='bold')
...
for scene in ["street", "campus", "indoor"]:
    ax.plot(metrics[scene]["iterations"], metrics[scene]["loss"], color=scene_colors[scene], label=scene_labels[scene], linewidth=2)
...
fig, ax = plt.subplots(figsize=(10, 6))
for scene in ["street", "campus", "indoor"]:
    ax.plot(metrics[scene]["iterations"], metrics[scene]["psnr"], color=scene_colors[scene], label=scene_labels[scene], linewidth=2)
ax.set_xlabel("Iteration", fontsize=12)
...
ax.set_xlim(0, 1); ax.set_ylim(0, 1); ax.axis('off')
fig.set_facecolor('#0d1117')
ax.text(0.5, 0.6, 'github.com/rsasaki0109/nerf-gs-playground', fontsize=18, ha='center', color='#58a6ff')
ax.text(0.5, 0.4, 'pip install -e . && gs-playground run --images data/', fontsize=13, ha='center', color='#8b949e', family='monospace')
save_frame(fig, 2000)
