...
| Multi-agent / moving obstacles | 単一の moving obstacle は `DynamicObstacleTimeline` で scenario config に入った (`step_index` に対して線形補間する waypointed sphere)。Multi-agent 相互作用 / reactive policy 側の連携は今後。 |
